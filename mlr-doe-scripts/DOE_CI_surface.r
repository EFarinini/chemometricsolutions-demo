...
if(DOE$loY==FALSE & res){
  ...
  if(dof>0){
    ...
    ir<-1
    q<-qt(0.975,dof)
    lev<-data.frame(Y=q*s*sqrt(diag(gr%*%disper%*%t(gr))),gr)
    lev<-lev[,c(1,v1+2,v2+2)]
    names(lev)<-c("z","x","y")
    print(paste('Semiamplitude: min. ',format(min(lev$z),digits=4),' average ',
                format(mean(lev$z),digits=4),' max. ',format(max(lev$z),digits=4),sep=''))
    dev.new(title=paste("confidence interval surface",DOE$Yname))
    zlab<-NULL
    ...
    confint.3D<-lattice::wireframe(z~x*y,data=lev,drape=TRUE,col.regions = colorRampPalette(c("yellow","green","blue"))(256),
                               ...
                               par.settings=list(
                                 par.sub.text = list(font = 1,cex=1,
                                                     ...
                                                     x = grid::unit(5, "mm"))),
                               sub=text.subtl,
                               cex.main=1,xlab=DOE$name[v1],
                               ylab=DOE$name[v2],zlab=paste('conf. int.')) 
    print(confint.3D)
    dev.new(title=paste("confidence interval contour plot",DOE$Yname))
    confint.cont<-lattice::contourplot(z~x*y,data=lev,cuts=15,main=paste("Semiampl. of the confidence interval",DOE$Yname),
                                       par.settings=list(
                                         par.sub.text = list(font = 1,cex=1,
                                                             just = "left", 
                                                             x = grid::unit(5, "mm"))),
                                       sub=text.subtl,
                                       cex.main=1,xlab=DOE$name[v1],ylab=DOE$name[v2],
                                       col='blue',labels=list(col="blue",cex=1))
