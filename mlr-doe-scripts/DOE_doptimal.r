...
previous.name<-"cp"
ans<-inpboxekk('*Matrix with Candidate Points','Model with :',c('Intercept','Higher Terms'))
int=ans[[2]]
...
if(sum(apply(x,2,is.numeric))==ncol(x)){
 ...
 x<-as.matrix(x)
 if(qr(t(x)%*%x)$rank<ncol(t(x)%*%x)){
   print("Impossible to find suitable subsets: the matrix is rank deficient")
   rm(x)
 }else{
   maxinfl<-NULL
   r<-nrow(x)
...
if(exists('ans'))rm(ans)
if(exists('variable'))rm(variable)
if(exists('int'))rm(int)
if(exists('d'))rm(d)
if(exists('dmax'))rm(dmax)
if(exists('h'))rm(h)
if(exists('j'))rm(j)
if(exists('l'))rm(l)
if(exists('matmod'))rm(matmod)
if(exists('maxt'))rm(maxt)
if(exists('xin'))rm(xin)
if(exists('xout'))rm(xout)
