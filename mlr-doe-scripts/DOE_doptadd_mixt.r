...
c('Quadratic Terms','Cubic Term'),c('','','TRUE','TRUE'))
...
if(!is.null(ans)){
	...
	tot<-matmod(tot,FALSE,ans[[3]])
	if(ans[[4]]){
	  tot<-cbind(tot,tot[,1]*tot[,2]*tot[,3])
	}
...
plot(logmt[1,1:w],logmt[2,1:w],col='red',type='b',xlab='Number of Additional Experiments',ylab='log(Normalized Determinant)')
	...
	if(miss<=5){
		 ...
		 rm(x,nlog,maxinfl,logmt,logm,mexp,o,j1,j2,levout,levin,co,r,n,w,ne,nt,d,dmax,h,j,l,matmod,maxt,nnt,ror,rtot,tot,totin,xin,xori,xout,miss)
	}
}
rm(ans)
