...
require(matrixcalc)
require(MASS)
if(!exists('doe.set'))doe.set<-c(previous.name,'all','','None','TRUE','TRUE')
...
c('Intercept','Higher Terms'),doe.set)
if(!is.null(ans)){
 previous.name<-ans[[1]]
 doe.set<-ans
 DOE<-list()
 ...
 if(ncol(x)!=length(coeff_name))coeff_name<-c('b0',coeff_name)
 ...
 if(ans[[5]]){
  ...
  print.table(format((inf1),digit=4),quote=FALSE)
  cat(' ',"\n")
 }
