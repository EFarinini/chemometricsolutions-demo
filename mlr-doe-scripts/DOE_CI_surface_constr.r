  ...
  if(!is.null(ans)){
    ...
    if(DOE$loY==FALSE & res){
      ...
      if(dof>0){
        ...
        Q=X%*%solve(t(P)%*%P)%*%t(X)
        q_<-qt(0.975,dof)
        Lev=data.frame(trian.cond,"L"=q_*s*sqrt(diag(Q)))
        ...
        p=lattice::contourplot(L~base*high, Lev, col = "red", cuts = 10,aspect = ((h2-h1)+.1)/((b2-b1)+.1),region = TRUE,col.regions = colorRampPalette(c("yellow","green","blue")),label=list(col="black",cex=0.9),
                               main="Semiampl. of the confidence interval",
        ...
        dev.new(title=paste("confidence interval contour plot mixture (zoom)",DOE$Yname))
        print(p)
